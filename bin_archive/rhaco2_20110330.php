...
class Flow extends Request{
	...
	private function set(&$module_obj,$map,$pattern,$params,$urls,$index=0,$method_call=false){
		...
		if($method_call){
			if($this->ext_template->filename() === null && !isset($map['template'])){
				$ref = new ReflectionObject($this);
				$file = dirname($ref->getFileName()).'/resources/templates/'.$map['method'].'.html';
				if(is_file($file)){
					$map['template'] = basename($file);
					$map['template_path'] = dirname($file);
		...
		if(!empty($map['template'])){
			...
			if($this->is_theme() || isset($map['theme_path'])){
				...
				$this->ext_template->media_url(
					File::absolute(
						$this->ext_template->media_url()
						,File::path_slash($map['theme_path'],false,true).File::path_slash($this->theme(),false,false)
				));
			}
			if(isset($map['template_path'])){
				$template = File::absolute(File::path_slash($map['template_path'],false,true),$template);
			}
