...
class App{
 ...
 static public function config_path($path,$url=null,$work=null,$mode=null,$vendors_path=null,$libs_path=null){
  ...
  if(!is_file(App::path('setup.php'))) Lib::config_download(false);
...
class Lib{
 ...
 static public function download($package,$loaded=array()){
  foreach(Rhaco2::repositorys() as $search_path){
   ...
   try{
    File::untgz($search_path.$package.'.tgz',self::$vendor_path);
    $loaded[$package] = $package;
       if(is_dir(self::$vendor_path.'/'.str_replace('.','/',$package))){
     foreach(File::ls(self::$vendor_path.'/'.str_replace('.','/',$package)) as $f){
      if($f->ext() == '.php'){
       if(preg_match_all('/[^\w]import\(([\"\'])(.+)\\1/',file_get_contents($f->fullname()),$m)){
        foreach($m[2] as $p){
...
class Object{
 ...
 final public function __set($n,$v){
  if(in_array($n,$this->_m[2])){
   $this->_ = array($this,$n);
   call_user_func_array(array($this,'__set__'),array($v));
 ...
 protected function __new__(){
  ...
  if(isset($_SERVER['REQUEST_METHOD'])){
   ...
   if(!empty($args) && strpos($args[0],'_request_=false') === false){
    $this->scope = preg_match("/scope=([\w_]+?)/",$args[0],$m) ? trim($m[1]) : get_class($this);
...
class Setup extends Object{
 ...
 static public function start(){
  ...
  try{
   ...
   if(!is_file($settings_path)){
    ...
    if(substr($jump_path,-9) !== '/rhaco2.php'){
     $jump_path = dirname($jump_path).'/rhaco2.php';
 ...
 static public function __setup_v__(Request $req,$value){
  if(empty($value)){
   ...
   $version = date('Ymd.His',File::last_update((substr($object_path,-9) === '/rhaco2.php') ? $object_path : constant('_JUMP_PATH_')));
...
class File extends Object{
 ...
 public function size(){
  return (@is_file($this->fullname)) ? @filemtime($this->fullname) : strlen($this->value);
 }
...
function module_const_array($name,$option=null){
 ...
 if(isset($option)){
  if(is_array($option)){
   ...
   Log::info($option,$option-sizeof($result));
...
Test::__import__();
Log::__import__();
if((php_sapi_name() == 'cli' || !isset($_SERVER['REQUEST_METHOD'])) && sizeof(debug_backtrace()) == 0) Setup::start();
