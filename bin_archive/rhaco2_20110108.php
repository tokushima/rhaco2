...
class Lib{
 ...
 static public function import($package){
  ...
  if(self::package_path($realpath) != $package) throw new InvalidArgumentException($package.' not found');
   ...
   foreach(File::ls(App::path()) as $f){
    if($f->is_ext('php')){
     ...
     if(Tag::setof($tag,$src,'app')){
      if(preg_match_all("/[^\w]class=([\"\'])([\w\.]+?)\\1/",$src,$match)){       
       foreach($match[2] as $path) $packages[trim($path)] = trim($path);
      }
     }
    }
   }
