...
class Flow extends Request{
	...
	private function set(&$module_obj,$map,$pattern,$params,$urls,$index=0,$method_call=false){
		...
		if(!empty($template)){
			...
			if(isset($template_path)){
				$template = File::path_slash($template_path,null,true).$template;
			}
							...
							foreach($handler->in(array('maps','map','var','module')) as $tag){
								switch(strtolower($tag->name())){
									...
									case 'maps':
										...
										$maps_media_path = ($tag->is_param('media_path') || isset($media_path)) ? File::path_slash($media_path,false,true).File::path_slash($tag->in_param('media_path'),false,false) : null;
	...
	static private function parse_map(Tag $map_tag,$is_url,$url,$template_path,$media_path,$theme_path,$scope,$base_class,$secure,$update,$map_index){
		...
		$params['error_template'] = File::path_slash($params['template_path'],null,true)
										.File::path_slash($map_tag->in_param('error_template'),false,false);
