...
class Image extends Object{
	...
	static public function load($filename){
		if(!is_file($filename)) throw new ImageException("file not found");
		$size = getimagesize($filename);
		if($size === false) throw new ImageException("invalid file");
		...
		try{
			...
			throw new ImageException("invalid data");
		}
		return $self;
	}
	...
	public function rotate($angle,$bg_color="#000000"){
		if($angle != 0){
			...
			if($resource === false) throw new ImageException("rotate fail");
			$this->resource = $resource;
		}
		return $this;
	}
	...
	public function write($filename,$type=null){
		...
		if(!$bool) throw new ImageException("invalid type");
		return $filename;
	}
	/**
	 * イメージを取得する
	...
	private function output_image($type=null){
		...
		throw new ImageException("invalid type");
	}
	/**
	 * 標準出力に出力する
	 *
