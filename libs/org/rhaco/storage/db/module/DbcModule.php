 ...
 * @author tokushima
 */
abstract class DbcModule extends Object{
	protected $encode;
	protected $quotation = '`';
	...
	public function connect($name,$host,$port,$user,$password,$sock){
		throw new PDOException('undef');
	}
	/**
	 * create table文を生成する
	 * @see org.rhaco.storage.db.Dbc::create_table
	 * @param string $name
	 * @param array $columns
	 * @return Daq
	 */
	...
	public function drop_table($name){
		return Dao::daq('drop table '.$this->quotation($name));
	}
	/**
	...
	/**
	 * show_columns_sqlの結果をパースする
	 * @param PDOStatement $it
	 * @return array ( object("name"=>"..","default"=>"..","annotation"=>"..");
	 */
	...
	public function select_sql(Dao $dao,Q $query,$paginator,$name=null){
		...
		return Dao::daq(('select '.implode(',',$select).' from '.implode(',',$from)
	...
	protected function which_aggregator_sql($exe,Dao $dao,$target_name,$gorup_name,Q $query){
		...
		list($where_sql,$where_vars) = $this->where_sql($dao,$from,$query,$dao->self_columns(true),$this->where_cond_columns($dao->conds(),$from));
		return Dao::daq(('select '.$exe.'('.$target_column->table_alias().'.'.$this->quotation($target_column->column()).') target_column'
